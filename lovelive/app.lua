...
function app.reload()
    myShader = love.graphics.newShader[[
      ...
      vec4 effect( vec4 color, Image texture, vec2 texture_coords, vec2 screen_coords ){
          ...
          float l = iGlobalTime / .5 + val1;
          float c = val1 / 200 / abs(1.0 / k - 2.0 - (tan(b-l+val1)));
          float d = 0.05 * abs(2.0 * k - 2.0 - (tan(10305.0 + b+l) + cos(10000035.0 * b - 1.0 * l) + tan(10035.0 * b - 1.0 * l)));
          float c2 = 0.05 / abs(5.0 * k - 2.0 - (tan(b-l)));
          vec4 i = vec4(0.13 - c2, c + 0.3 * c2 , 0.0, 1.0) + vec4(d, 0.0, 1.0, 1.0);
          return i;
      }
    ]]
end
...
function app.update(dt)
  ...
  if data then 
    ...
    music:setPosition(math.random(data)/100, 0, math.random(data)/100)
    music:setPitch(.8)
  end
